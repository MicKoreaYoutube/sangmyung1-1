              ...
              <nav className="nav-flex items-center space-x-2">
                {suggestions_list.map((suggestion, index) => (
                  <div key={index} className="flex items-center h-full">
                    <h3 className="text-xl text-center">{suggestion.title} ·<span className="text-sm text-gray-400">{suggestion.author}</span>·<span className="text-sm text-gray-400">{suggestion.changeTime.toLocaleString()}</span></h3>
